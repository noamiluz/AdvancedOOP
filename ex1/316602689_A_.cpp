...
Direction _316602689_A::step(Direction prevStep) {
	...
	if (m_needs_to_return /*1*/){
		...
		if (s_i_.dirtLevel > 0){ // current position still dirty
			return Direction::Stay;
		}
			...
			if (d == Direction::Stay){// i.e - the algorithm finished cleaning, bfs to 'D' just at the first time the algo
				// finishes cleaning.
...
**/
void _316602689_A::set_battery_level(){
...
**/
void _316602689_A::update_scan_house_map(SensorInformation info){
...
**/
map<pair<int, int>, _316602689_A::Vertex*> _316602689_A::create_graph_from_scan_map(map<pair<int, int>, char>& scan_map){
	...
	{
		if (it->second != 'W'){
			graph[{it->first.first, it->first.second}] = new _316602689_A::Vertex();
		}
...
**/
Direction _316602689_A::bfs(map<pair<int, int>, _316602689_A::Vertex*> graph, pair<int, int> s, _316602689_A::Vertex* t, bool only_un_scanned_nodes){
	// do the bfs
	t->m_color = 'g';
	t->m_distance = 0;

	map<pair<int, int>, _316602689_A::Vertex*>::iterator it;
	_316602689_A::Vertex *source = nullptr, *cur = nullptr, *un_scaaned_node = nullptr;
	queue<Vertex*> q;
	q.push(t);
	bool found_s = false;
...
**/
Direction _316602689_A::check_if_needs_to_return(int batteryLeft_or_maxStepsAfterWinner){
	// create graph from the scaned map and run bfs
...
extern "C"{
	REGISTER_ALGORITHM(_316602689_A)
}